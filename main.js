...
async function runAutomation() {
  ...
  try {
    ...
    // 1. Add new channels from config to the DB
    for (const url of configChannels) {
      await db.run('INSERT OR IGNORE INTO channels(url) VALUES(?)', url);
    }

    // 2. Remove channels from the DB that are no longer in the config
    ...
    for (const channel of channelsToRemove) {
      ...
      // We must first delete dependent videos to maintain database integrity
    ...
    // Get the final, correct list of channels to process
    ...
    // This part is important if you run the script without the scheduler
    if (!ENABLE_SCHEDULER) {
        await db.close();
    }
  }
...
async function downloadAndProcessVideo(videoInfo) {
  ...
  try {
    ...
    const finalOutputFile = join(channelDir, `${sanitizedTitle}.amv`);
    ...
    await transcodeToAmv(tempVideoFile, tempAudioFile, finalOutputFile);
    console.log(chalk.green(`✅ Success! Video saved as ${finalOutputFile}`));
  } catch (error) {
    console.error(
...
async function transcodeToAmv(videoPath, audioPath, outputPath) {
  const args = [
    '-y',
    '-i',
    videoPath,
...
if (ENABLE_SCHEDULER) {
  // If scheduler is enabled, set it to run twice a day.
  ...
  // If scheduler is disabled, just run the script once and exit.
